...
do
    ...
    # Sending SIGKILL to docker image that contains realsense nodes
	echo "Killing container"
	ssh $EXP_HOST "cd $server_test_path && ./realsense_stop_all_containers.sh" >> $log_experiment 2>> $log_experiment &
	sleep 5
	...
	ssh $EXP_HOST "cd $server_test_path && ./realsense_restart_on_missing_camera.sh" >> $log_experiment 2>> $log_experiment
